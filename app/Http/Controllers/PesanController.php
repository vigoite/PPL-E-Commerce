    ...
    public function konfirmasi(Request $request)
{
    ...
    $pesanan = Pesanan::where('user_id', Auth::user()->id)->where('status', 0)->first();
        if (!empty($pesanan)) {
            ...
            $pesanan->kurir = $request->courier_hidden;
            $biaya_ongkir = $pesanan;
            $pesanan->update();
            // Simpan nama kurir
            
            // Periksa apakah $pesanan->pesananDetails adalah array atau objek
            if (is_array($pesanan->pesananDetails) || is_object($pesanan->pesananDetails)) {
                foreach ($pesanan->pesananDetails as $pesanan_detail) {
